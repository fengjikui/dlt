...
def test_context_allow_external_schedulers_flag(
    ...
    """User-set `allow_external_schedulers` wins over context; context `True` sets it on
    unset (None) incrementals at bind time, `False` does nothing."""
    ...
    # ctx True fills the unset flag on the instance, otherwise user setting is preserved
    expected_aes = incr_aes if incr_aes is not None else (True if ctx_aes else None)
    assert inc.allow_external_schedulers is expected_aes
