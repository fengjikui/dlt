...
def test_migrate_manifest_v1_to_v2() -> None:
    """Engine 2 replaces `allow_external_schedulers` with `incremental_mode` and
    `refresh` with `refresh_propagation`."""
    ...
    assert "allow_external_schedulers" not in jobs["jobs.events.hourly_events"]
    # no refresh policy in the source job
    assert "refresh_propagation" not in jobs["jobs.events.hourly_events"]
    # explicit flag False dropped without setting a mode
    assert "incremental_mode" not in jobs["jobs.events.daily_report"]
    assert "allow_external_schedulers" not in jobs["jobs.events.daily_report"]
    # refresh policies converted and dropped
    ...
    assert "refresh" not in jobs["jobs.events.cleanup"]
    # job without the flag untouched
    assert "incremental_mode" not in jobs["jobs.events.cleanup"]
